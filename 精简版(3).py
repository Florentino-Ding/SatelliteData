...
def evaluate_coverage(pred_df):
    # 读取原始数据文件来获取2023年的累计发射数据
    df = pd.read_csv("satellite_data.csv")
    
    # 获取2023年各国累计发射总量(stock_launch)并排序获取前20国家
    top_20_countries = df[df['year'] == 2023].sort_values('stock_launch', ascending=False)['country'].head(20).tolist()
    
    # 只保留前20国家的数据
    valid = pred_df[pred_df['country'].isin(top_20_countries)].dropna(subset=['flow_launch_actual'])
    ...
    for idx, row in valid.iterrows():
        if pd.isna(row['flow_launch_pred_lower']) or pd.isna(row['flow_launch_pred_upper']):
            continue
        total += 1
        if row['flow_launch_actual'] >= row['flow_launch_pred_lower'] and row['flow_launch_actual'] <= row['flow_launch_pred_upper']:
            coverage_count += 1
...
def evaluate_candidate(args):
    ...
    try:
        ...
        if valid.empty:
            ...
            error = (valid['flow_launch_pred_median'] - valid['flow_launch_actual']).abs().mean()
    except Exception as e:
        logger.error(f"Error evaluating candidate tau={tau:.7f}, gamma={gamma:.7f}: {e}")
        coverage = 0
        error = float('inf')
...
if __name__ == "__main__":
    ...
    if os.path.exists(checkpoint_file):
        ...
        processed_set = set(checkpoint_df.apply(lambda row: f"{float(row['tau']):.7f}_{float(row['gamma']):.7f}", axis=1).tolist())
    ...
    # 定义候选参数范围（可根据需要调整）
    tau_candidates = np.linspace(1e-9, 1e-10, 10)
    gamma_candidates = np.linspace(0.0001, 0.000001, 10)
    ...
    candidates = [(tau, gamma, n_sims_for_mc) for tau, gamma in itertools.product(tau_candidates, gamma_candidates)
                  if f"{tau:.7f}_{gamma:.7f}" not in processed_set]
        ...
        with concurrent.futures.ProcessPoolExecutor() as executor:
            ...
            for future in tqdm(concurrent.futures.as_completed(future_to_candidate),
                ...
                try:
                    ...
                    continue
                key = f"{tau_val:.7f}_{gamma_val:.7f}"
                if coverage > best_score:
                    best_score = coverage
                    best_tau = tau_val
                    best_gamma = gamma_val
                # 写入当前候选组合结果并立即刷新
                checkpoint_writer.writerow([f"{tau_val:.7f}", f"{gamma_val:.7f}", coverage, error])
                checkpoint_f.flush()
                logger.info(f"候选 tau={tau_val:.7f}, gamma={gamma_val:.7f} -> 覆盖率: {coverage:.2%}, 误差: {error:.4f}")
    ...
    logger.info(f"\n最佳参数（基于覆盖率）为：")
    logger.info(f"tau = {best_tau:.7f}, gamma = {best_gamma:.7f}，覆盖率 = {best_score:.2%}")
    
    # 保存预测误差最小的15个候选组合，同时保存其覆盖率与误差
    checkpoint_df = pd.read_csv(checkpoint_file)
    checkpoint_df = checkpoint_df[checkpoint_df['error'] != float('inf')]
    top15 = checkpoint_df.sort_values(by="error", ascending=True).head(15)
    top15.to_csv("top15_candidates.csv", index=False)
    logger.info("保存预测结果和实际结果差异最小的15个候选组合至文件：top15_candidates.csv")
    
    # 使用最优参数运行完整流程（增加蒙特卡洛模拟次数以获得更稳健的预测）
    try:
        final_pred_2024 = run_pipeline(best_tau, best_gamma, n_sims_for_mc=100)
